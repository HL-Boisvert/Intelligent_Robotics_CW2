...
class Controller:
    ...
    def calculate_fitness(self):
        ...
        # define the binarization threshold fro ground sensors
        min_gs = 345
        max_gs = 620
        # normalise the ground sensors
        ...
        # Normalize the values save data
        left = (left-min_gs)/(max_gs-min_gs)
        center = (center-min_gs)/(max_gs-min_gs)
        right = (right-min_gs)/(max_gs-min_gs)
        
        # get proximity sensors values
        proxValues = []
        for i in range(2):
            sens_01 = self.proximity_sensors[i].getValue()  
            sens_02 = self.proximity_sensors[7-i].getValue()               
            # normalise the proximity sensors
            min_ds = 90
            max_ds = 150
            if(sens_01 > max_ds): sens_01 = max_ds
            if(sens_01 < min_ds): sens_01 = min_ds
            if(sens_02 > max_ds): sens_02 = max_ds
            if(sens_02 < min_ds): sens_02 = min_ds
            #
            proxValues.append(sens_01)
            proxValues.append(sens_02)
            
            
        ### Define the fitness function to increase the speed of the robot and 
        ### to encourage the robot to move forward only
        # Check if the denomitor is not equal to 0
        maxDenom = max(self.velocity_left,self.velocity_right)
        # If so the denomitor is equal to 1
        if( maxDenom == 0): maxDenom = 1 
        forwardFitness = min(self.velocity_left,self.velocity_right) / abs(maxDenom)
        
        ### Define the fitness function to encourage the robot to follow the line
        followLineFitness = 1 - math.sqrt((right+center+left)/3)
                
        ### Define the fitness function to avoid collision
        avoidCollisionFitness = 1 - math.sqrt((max(proxValues)-min_ds)/(max_ds-min_ds))

        ### Define the fitness function to avoid spining behaviour
        spinningFitness = pow(2,abs(self.velocity_left - self.velocity_right)) /pow(2,2*self.max_speed)
